...
>
> **【2026.06 追記・トーン変更】** 本部より柔らかい**ポップ・親しみやすい**方向に決定（オーナー）。
> ・文章は **です・ます**中心で平易に。大げさ/中二病的な見出し（例「動いている事実」）は使わない。
> ・**角丸は大きめ**（カード 12〜16px、ボタン・バッジはピル型 OK）。§7 の「角丸4〜6px・ピル禁止」は無効。
> ・ヒーロー・CTA帯は**濃紺ではなく明るい水色**。サイト全体を明るく軽く。
> ・**連携/提携は前面に出さない**（トップに専用セクションを置かない。contact に控えめに）。
> ・グラデ禁止・アクセント一色（ブルー）は引き続き維持。
>
> **【2026.07 更新・デザイン言語とロゴ実測色（オーナー決定）】**
> ・デザインの第一参考は **uniways.co.jp**（曲線リボン背景 / 白い浮遊パネル / 大きな青い英字見出し /
>   白ピル型ナビ / 濃紺フッター）。トーンは「信頼感のあるプロフェッショナル × 明るく前向き」。
> ・配色を**本部ロゴの実測色**に変更: 水色 `--brand: #83cbe4`（装飾・リボン・面）×
>   濃紺 `--ink: #22374a` × 白。対話要素（CTA・リンク）は導出色 `--accent: #1e7fa8`（白地4.5:1 AA）。
>   旧ブルー `#1f6ab0` は廃止。§2 のトークン表より `css/style.css` の `:root` が正。
> ・本部公式ロゴPNGを取得済み: `images/logo-icon.png`（ヘッダー）/ `images/favicon-192.png`（ファビコン）。
> ・コンテンツの一次資料は「学生部 行動指針」PDF（2026/7/1）。スローガン・3指針・基本サイクル・行動宣言に準拠。
